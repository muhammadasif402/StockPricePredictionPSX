...
max_date = datetime.datetime(2023, 8, 31)
...
# Initialize the DataFrame outside the confirmation block
df = None
...
# Create a button for future prediction
predict_button = st.sidebar.button("Predict Future Price")

# When the button is clicked, trigger the prediction function
if predict_button:
    if df is not None:
        # Get the most recent data point for prediction
        last_data_point = df['Close'].iloc[-1]
    
        # Prepare the input data for the model to predict the future price
        input_data = scaled_data[-60:]  # Use the last 60 data points
    
        # Reshape the input data for the model
        input_data = np.reshape(input_data, (1, input_data.shape[0], 1))
    
        # Use the model to predict the next month's price
        ...
        # Display the predicted future price and date
        st.sidebar.subheader('Future Price Prediction')
        st.sidebar.write(f"Predicted Close Price for {next_month}: ${future_price[0][0]:.2f}")
